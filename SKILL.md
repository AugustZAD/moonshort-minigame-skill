...
| Layer 3 前景素材和圆形/方形容器硬切，边缘看起来像贴纸剪出来 | PNG 直接塞进容器，没做边缘淡入处理。只加 `mix-blend-mode: screen` 不够 —— 在 stacking context 里 screen 只能和容器内兄弟元素混合，看不到外层 cover-layer，PNG 的黑/绿角色照样画成硬色块 | **双层防线**（必须都做）：① PNG 生成时走 `applyVignette()` 后处理（`scripts/generate-layer3-assets.js`）用 `inner: 0.55 / outer: 1.0` 并用 `halfMin = min(W,H)/2` 归一化（不是 halfDiag，否则边中点还在 rolloff 带内就会看到方形轮廓）。② 容器 img 加 `mask-image: radial-gradient(circle closest-side at center, #000 30%, rgba(0,0,0,.88) 55%, rgba(0,0,0,.55) 76%, rgba(0,0,0,.2) 92%, transparent 100%)` 5 段软淡入。**不要用 `mix-blend-mode: screen`**，因为 stacking context 隔离 |
| Layer 3 chroma key 扣完主体变成"鬼影镂空" | 主体颜色（verdigris 绿青铜、草绿、浅绿）被 chroma key 当绿幕扣掉，背景透过来 | 主体颜色有绿/青/黄时**禁用 chroma key**，改走 vignette 路径：Gemini prompt 不画 vignette（"do not add vignette, will be added in post-processing"），直接在脚本里 `applyVignette()` 乘到纯黑，用 `mask-image` 做 CSS 层软淡入。另外 prompt 里也要避开 "verdigris-green" 字样，改成 "weathered dark bronze with blue-grey oxidation + teal patina highlights"，减少 Gemini 对绿色元素的强调 |
| Layer 3 写了一版 chroma key feather 结果可见绿色反而更多 | Pass 1 只把强绿像素 `alpha=0` 但保留了 RGB 为 `#00FF00`，Pass 2 alpha 高斯 blur 向外扩散到这些像素，绿色作为低 alpha 像素重新可见 | Pass 1 必须同时清零 RGB（`pixels[i]=pixels[i+1]=pixels[i+2]=0`），这样 blur 外扩的 alpha 只会显示出柔和深色阴影，不会亮绿回归 |
| Layer 3 chromaKey 跑两次就把主体吃掉一半 | chromaKey 有 1px erode + blur，不是幂等的。连跑两次让不透明区缩水一半 | chromaKey 只能对**原始 Gemini raw buffer** 调用一次。如果要重做必须 `--force` 重新调 API，不能在已处理过的 PNG 上复跑 |
| Layer 3 前景还有青色荧光/边框 | 忘了清理 state-specific CSS 里的 `filter: drop-shadow(... var(--primary) ...)` 和 `.ring` 装饰环 | 纯素材融入场景的目标下：① `.ring / .glow` 类 `display: none !important` ② state CSS 只保留 `opacity` 和 `transform: scale(...)`，**删掉所有 `filter: drop-shadow(... primary ...)`** ③ 不加 keyframe 脉冲动画（那会把 primary 色带出来） |
