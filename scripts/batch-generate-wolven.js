...
(function() {
  ...
  gs.update = function(time, dt) {
    ...
    // ── lane-dash: player + obstacle + orb sprites (V4 Canvas-compatible) ──
    // NOTE: Game uses Phaser.CANVAS renderer — setTintFill does NOT work on Canvas!
    // Use Phaser Circle/Ellipse game objects for colored glows instead.
    if (templateId === 'lane-dash') {
      spritePatch += `
(function() {
  var gs = GameScene.prototype || GameScene;
  var W = 393, H = 852;
  // Canvas-compatible glow: two concentric Phaser circles (outer diffuse + inner bright)
  function addCircleGlow(scene, x, y, r, color, depth) {
    var outer = scene.add.circle(x, y, r * 1.5, color, 0.12).setDepth(depth - 0.1);
    var inner = scene.add.circle(x, y, r, color, 0.45).setDepth(depth);
    return { inner: inner, outer: outer };
  }
  function moveGlow(g, x, y) { if (g) { if (g.inner) g.inner.setPosition(x, y); if (g.outer) g.outer.setPosition(x, y); } }
  function destroyGlow(g) { if (g) { if (g.inner) g.inner.destroy(); if (g.outer) g.outer.destroy(); } }
  function cleanObj(o) {
    if (o._spr) { o._spr.destroy(); o._spr = null; }
    if (o._spr2) { o._spr2.destroy(); o._spr2 = null; }
    destroyGlow(o._g1); o._g1 = null;
    destroyGlow(o._g2); o._g2 = null;
  }
  var origCreate = gs.create;
  gs.create = function() {
    origCreate.call(this);
    // Cover image as semi-transparent game background
    if (this.textures.exists('ep_bg_game')) {
      var bg = this.add.image(W / 2, H / 2, 'ep_bg_game');
      var src = bg.texture.getSourceImage();
      bg.setScale(Math.max(W / src.width, H / src.height)).setAlpha(0.18).setDepth(0.1);
    }
    // Player sprite with primary-color circle glow
    if (this.textures.exists('ep_sprite_player') && this.player) {
      var pInt = parseInt((window.__V3_THEME__.primary || '#4ECDC4').replace('#',''), 16);
      this._pG = addCircleGlow(this, this.player.x, 700, 32, pInt, 10);
  ...
  gs.update = function(time, dt) {
    ...
    if (this._sprPlayer && this.player) {
      ...
      if (this._pG && this._pG.inner) this._pG.inner.setAlpha(0.4 + Math.sin((this._glowPhase || 0) * 1.5) * 0.1);
    }
    // Obstacles — bright RED circle glow behind sprite (Canvas-safe)
    if (this.hazards && hasObsSpr) {
      for (i = 0; i < this.hazards.length; i++) {
        ...
        if (!h._spr && h.y > 0) {
          ...
          if (h.type === 'wide') {
            h._g1 = addCircleGlow(this, h.x - 40, h.y, 34, hc, 8);
            h._g2 = addCircleGlow(this, h.x + 40, h.y, 34, hc, 8);
            ...
            h._g1 = addCircleGlow(this, h.x, h.y, 34, hc, 8);
        ...
        if (h._spr) {
          if (h.type === 'wide') {
            ...
            moveGlow(h._g1, h.x - 40, h.y); moveGlow(h._g2, h.x + 40, h.y);
          } else {
            h._spr.setPosition(h.x, h.y); moveGlow(h._g1, h.x, h.y);
          }
          if (h.y > 900) cleanObj(h);
        }
      }
    }
    // Orbs — bright GREEN circle glow behind sprite, pulsing (Canvas-safe)
    if (this.orbs && hasOrbSpr) {
      for (var j = this.orbs.length - 1; j >= 0; j--) {
        var orb = this.orbs[j];
        if (orb.collected || orb.y > 800) { cleanObj(orb); continue; }
        if (!orb._spr) {
          orb._g1 = addCircleGlow(this, orb.x, orb.y, 24, 0x44FF88, 7);
        ...
        moveGlow(orb._g1, orb.x, orb.y);
        if (orb._g1 && orb._g1.inner) {
          orb._g1.inner.setScale(sc * 1.1); orb._g1.outer.setScale(sc * 1.5);
          orb._g1.inner.setAlpha(0.4 + Math.sin(orb.phase || 0) * 0.15);
        }
      }
    }
  };
