...
(function() {
  ...
  function destroyGlow(g) { if (g) { if (g.g1) g.g1.destroy(); if (g.g2) g.g2.destroy(); if (g.g3) g.g3.destroy(); } }
  function cleanObj(o) {
    if (o._spr) { o._spr.destroy(); o._spr = null; }
    if (o._spr2) { o._spr2.destroy(); o._spr2 = null; }
    destroyGlow(o._gl); o._gl = null;
    destroyGlow(o._gl2); o._gl2 = null;
  }
  var origCreate = gs.create;
  gs.create = function() {
    origCreate.call(this);
  ...
  gs.update = function(time, dt) {
      ...
      for (i = 0; i < this.hazards.length; i++) {
        ...
        if (h._spr) {
          if (h.type === 'wide') {
            ...
            moveGlow(h._gl, h.x - 40, h.y); moveGlow(h._gl2, h.x + 40, h.y);
          } else {
            h._spr.setPosition(h.x, h.y); moveGlow(h._gl, h.x, h.y);
          }
    ...
    if (this.orbs && hasOrbSpr) {
      for (var j = this.orbs.length - 1; j >= 0; j--) {
        ...
        if (!orb._spr) {
          orb._gl = addGlow(this, orb.x, orb.y, '_glow_orb', 40, 40, 7);
          orb._spr = this.add.image(orb.x, orb.y, 'ep_sprite_collect').setDisplaySize(40, 40).setOrigin(0.5).setDepth(8);
        }
        ...
        if (orb._gl && orb._gl.g1) {
          orb._gl.g1.setDisplaySize(os * 1.08, os * 1.08);
          orb._gl.g2.setDisplaySize(os * 1.22, os * 1.22);
          orb._gl.g3.setDisplaySize(os * 1.4, os * 1.4);
