# 🎮 Mini-Game UI/UX Design System Prompt
...
所有 H5 小游戏统一使用 **Phaser 3** 作为游戏引擎，通过 CDN 引入：
...
**框架约束**：
- 每个小游戏为独立的单 HTML 文件，内联所有 JS/CSS
- 使用 Phaser Arcade Physics（轻量物理引擎）
- 贴图优先通过 `scene.textures.createCanvas()` 代码生成，减少外部资源依赖
- 画布尺寸遵循设计规范：393×736.5px，使用 `Phaser.Scale.FIT` + `CENTER_BOTH` 适配
...
小游戏通过 Cocos Creator 的 WebView 组件加载。游戏结束时，通过 JS Bridge 将评级结果传回宿主应用。
...
notifyGameComplete({
  rating: "A",        // "S" | "A" | "B" | "C"
  score: 8500,
  attribute: "Charm", // 由宿主通过 URL 参数传入
  modifier: +1        // S:+2 / A:+1 / B:0 / C:-1
});
```
...
webviewNode.on(WebView.EventType.JS_BRIDGE, (event: { detail: string }) => {
  ...
  // result.rating / result.score / result.attribute / result.modifier
});
```

**宿主向游戏传参**（通过 URL query）：

```
game.html?attribute=Charm&primaryColor=%23FF6B9D
```
...
const primaryColor = params.get('primaryColor') || '#FF6B9D';
```
...
- 游戏标题：`≥ 28px`，字重 `900`
- 评级字母：`≥ 64px`，字重 `900`
- 区块小标题：`18px ~ 22px`，字重 `700`
- 数值 / 标签：`13px ~ 16px`，字重 `700`
- 正文说明：`14px ~ 15px`，字重 `400`
- 最小文字：`≥ 11px`，字重 `400`
...
- **S** — `#FFD700` 金色，金色粒子爆炸 + 描边脉冲，`+2 Bonus · Lasts 1 Round`
- **A** — `primary` 主题色，星星粒子飘散，`+1 Bonus · Lasts 1 Round`
- **B** — `#9CA3AF` 中性灰，温和光圈，`No Change · Lasts 1 Round`
- **C** — `#F87171` 柔红，卡片轻微抖动，`-1 Penalty · Lasts 1 Round`
...
**开发规范**：
- 属性名称由宿主通过 URL 参数传入（见 §0 WebView Bridge），小游戏本身不硬编码
- 游戏结束时通过 `notifyGameComplete()` 将评级结果经 JS Bridge 传回宿主（见 §0 WebView Bridge）
- 评级结算界面的「继续」按钮点击后触发通信，不自动发送
